...
The output is committed, so nobody needs to run this. Deterministic via --seed.
"""
import argparse
import json
import random
...
FAILS = {
    ...
    "gaudi": ["gaudi oven never reached temperature"],
...
def gen_log(rng, idx):
    ...
    for n, step in enumerate(STEPS, start=1):
        ...
        if fail_at == n:
            # The verdict line repeats the error on purpose: one line per log
            # holds the bake number, the runtime, and why it failed, so a
            # single search across data/logs picks up all three.
            err = rng.choice(FAILS[oven] + FAILS_ANY)
            lines.append(f"[{t:05d}s] ERROR: {err}")
            lines.append(f"=== bake #{idx:04d} BURNT after {t}s: {err} ===")
            return "\n".join(lines) + "\n"
